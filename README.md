...
移动端使用竖屏模式：手指可从左侧约 68% 的区域任意位置落下并相对滑动，无需对准摇杆；控制采用 0.72 灵敏度、72px 目标前置上限和 232px/s 最高速度，兼顾灵活移动与精细躲弹，不会瞬移追手指。右侧释放导弹和炸弹，暂停键位于顶部 HUD 安全线下方。画布会根据移动浏览器的实际可视区域自动缩放；横屏时会提示旋转回竖屏。
